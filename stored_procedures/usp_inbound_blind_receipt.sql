USE [MyInventory]
GO
/****** Object:  StoredProcedure [inv].[usp_inbound_blind_receipt]    Script Date: 8/6/2569 13:40:50 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    BEGIN TRY
        ...
        IF @v_bit_is_base_uom = 1
            SET @v_dec_base_qty = ROUND(@in_dec_qty, @Round);

        ELSE
            SET @v_dec_base_qty = ROUND(@in_dec_qty * @v_dec_conv_factor, @Round);
        ...
        SELECT TOP 1
            ...
            @v_vch_next_line_number  = line_number
        ...
        BEGIN
            ...
            -- หา line_number ล่าสุดแล้วบวก 1

            SELECT @v_int_max_line = MAX(CAST(ISNULL(line_number, '0') AS INT))
            FROM [inv].[t_inv_inbound_detail]
            WHERE inbound_master_id = @in_int_inbound_master_id;

            SET @v_vch_next_line_number = RIGHT('00000' + CAST(ISNULL(@v_int_max_line, 0) + 1 AS VARCHAR), 5);
        ...
        VALUES (
            ...
            GETDATE()
                ...
                SET order_status = 'Receiving',
                        ...
                        update_date  = GETDATE()
                WHERE inbound_master_id = @in_int_inbound_master_id
                    AND ISNULL(order_status,'Open') NOT IN ('Receiving','Closed');
        ...
        USING (
            SELECT
                ...
                CAST(GETDATE() AS DATE)     AS receive_date
        ) AS source
        ...
        BEGIN
            ...
            WHERE inv.warehouse_id    = @v_int_warehouse_id
              ...
              AND ISNULL(inv.receive_date, '')          = ISNULL(CAST(GETDATE() AS DATE), '');
